...
<div id="joinDialog" class="dialogPanel no-select">
    ...
    <div id="join-container" class="content-container">
        ...
        -->
        <div id="joinPrompt">
            <div id="joinPromptTitle">${L("Settings")}</div>
            <div id="joinPromptBlurb" class="promptBlurb">${L("Specify a nickname, and choose an avatar either by selecting from those on display or pasting a valid Ready Player Me URL.")}</div>
        </div>
        <div id="settings-title" class="panel-title">${L("Settings")}</div>
        <div class="settings-container">
            <div id="nameInput" class="stringInputHolder">
                <div id="namePrompt" class="namePrompt">${L("Nickname")}<span>*</span></div>
                <div id="nameField" class="nameField allow-select" contenteditable="true"></div>
                <div id="nameExplanation">${L("Enter 1-12 characters (ASCII only).")}</div>
                <div id="nameFilterWarning"><br /></div>
            </div>
            <div id="selectAvatar" class="namePrompt">${L("Select Avatar")}</div>
            <div id="dialogAvatarSelections">
                <div id="avatarList"></div>
            </div>
            <div id="avatarURL" class="stringInputHolder">
                <div id="avatarURLPrompt" class="namePrompt">${L("Or, Enter an Avatar URL")}</div>
                <div id="avatarURLField" class="nameField avatarNameField allow-select" contenteditable="true"></div>
            </div>
            <div id="handednessRow">
                <div id="handednessLabel">${L("Hand")}:</div>
                <div class="btn-group" id="handedness">
                    <label class="btn btn-radio-button">
                         <input type="radio" name="options" id="left"><span class="handedness-label">${L("Left")}</span>
                    </label>
                    <label class="btn btn-radio-button">
                        <input type="radio" name="options" id="right" checked><span class="handedness-label">${L("Right")}</span>
                    </label>
                </div>
            </div>
            <div id="dialogEnterButton" class="dialogButtonsHolder disabled">
                <div id="enterButton">${L("Enter")}</div>
            </div>
            <div id="dialogAcceptCancelButtons" class="dialogButtonsHolder twoItems">
                <button id="cancel-button" type="button" class="btn btn-danger cancel-button">${L("Cancel")}</button>
                <button type="button" id="acceptButton" class="btn btn-success">${L("Apply")}</button>
            </div>
        </div>
    </div>
</div>
`.trim();
    ...
    if (joinPromptBlurb && simplerMenu) {
        joinPromptBlurb.textContent = L("Specify a nickname and press Enter.");
    }
...
function createShareMenu(avatar) {
    ...
    <div id="shareDialog" class="dialogPanel no-select">
        ...
        <div id="share-container" class="content-container">
            <div id="share-title" class="panel-title">${L("Invite Users")}<br></div>
            <div class="promptBlurb">${L("Scan QR code or click to open a new browser tab in the same session.")}</div>
            <div id="share-qr"></div>

            <div class="share-settings-label">${L("Copy Share Link")}</div>
            <div class="share-menu-row">
                <div id="copy-link" class="copy-link allow-select">${L("generated link")}</div>
                <button id="copy-button" type="button" class="btn btn-outline-success">${L("Copy")}</button>
            </div >
        <div id="save-vrse-row" class="share-menu-row">
            <div class="share-settings-label">${L("Save world as VRSE file")}</div>
            <button id="save-button" type="button" class="btn btn-outline-success">${L("Download")}</button>
        </div>
        </div >
    </div > `.trim();
